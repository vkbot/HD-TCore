...
bool WorldObject::IsWithinLOSInMap(const WorldObject* obj) const
{
    ...
    return (IsWithinLOS(ox, oy, oz) && GetMap()->IsInDynLOS(GetPositionX(), GetPositionY(), ox, oy));
}
...
bool WorldObject::IsWithinLOS(float ox, float oy, float oz) const
{
    ...
    return (vMapManager->isInLineOfSight(GetMapId(), x, y, z+2.0f, ox, oy, oz+2.0f) && GetMap()->IsInDynLOS(GetPositionX(), GetPositionY(), ox, oy));
}
...
void WorldObject::MovePositionToFirstCollision(Position &pos, float dist, float angle)
{
    ...
    while (!GetMap()->IsInDynLOS(pos.m_positionX, pos.m_positionY, destx, desty))
    {
